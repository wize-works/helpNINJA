    ...
    for (const row of rows) {
        try {
            ...
            if (row.integration_id) {
                const ds = await query<IntegrationRecord>(`select * from public.integrations where id=$1 and tenant_id=$2 and status='active'`, [row.integration_id, tenantId])
                integ = ds.rows[0] ?? null
            }
            if (!integ) {
                if (row.provider === 'email' && process.env.SUPPORT_FALLBACK_TO_EMAIL) {
                    integ = { id: 'env-email', tenant_id: tenantId, provider: 'email', name: 'env-email', status: 'active', credentials: {}, config: { to: process.env.SUPPORT_FALLBACK_TO_EMAIL, from: process.env.SUPPORT_FROM_EMAIL } }
                } else if (row.provider === 'slack' && process.env.SLACK_WEBHOOK_URL) {
                    integ = { id: 'env-slack', tenant_id: tenantId, provider: 'slack', name: 'env-slack', status: 'active', credentials: { webhook_url: process.env.SLACK_WEBHOOK_URL }, config: {} }
                }
            }
            if (!integ) {
